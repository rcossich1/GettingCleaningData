##run_analysis code run to accomplish:
...
## 4. step done before as keep work simple
## naming columns in training and test data sets
names(X_train) <- features$feature_name
names(X_test) <- features$feature_name

## 1. 
## merging data sets' rows
X_all<- rbind(X_train,X_test)

## 2.
## Extracting the mean() and standard deviation variables from the data set
X_all_meanstd <- select(X_all,grep("mean|std",names(X_all)))

## 3.
## reading activity set and code for descriptive naming
